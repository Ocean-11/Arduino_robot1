...
void setup() 
{
  ...
  // Setup motor PWM
  //analogWrite(DRIVE_RIGHT_PWM,120); //do 50% PWM on pin 11 at the frequency set in TCCR2B(right wheel) 
  //analogWrite(DRIVE_LEFT_PWM,120); 
  ...
  // Setup the servo
  myservo.attach(SERVO_SPARE);

  // Setup 2 US sensors
/*  pinMode(US_RIGHT_PING, OUTPUT);
  pinMode(US_RIGHT_ECHO, INPUT);
  pinMode(US_LEFT_PING, OUTPUT);
...
void loop() 
{
  ...
  { 
    ...
    // Poll the US sensors
//    us_measure(US_RIGHT_PING, US_RIGHT_ECHO, print_en);
    ...
    //PID_motor_control_r(float(req_velocity), current_right_vel, 5.0, 0.0, 0.0, DRIVE_RIGHT_PWM); 
    //PID_motor_control_l(float(req_velocity), current_left_vel, 5.0, 0.0, 0.0, DRIVE_LEFT_PWM); 
    
    if (is_feeder_active)
    {
      //motor_control_r.update_PWM(float(req_velocity), current_right_vel, 5.0, 0.0, 0.0,DRIVE_RIGHT_PWM); 
      //motor_control_l.update_PWM(float(req_velocity), current_left_vel, 5.0, 0.0, 0.0,DRIVE_LEFT_PWM); 
...
void PID_motor_control_r(float required_velocity, float current_velocity, float P, float D, float I)
{
  ...
  // Update the PWM
  analogWrite(DRIVE_RIGHT_PWM,required_pwm); // try to use HPWM!!!!!!!!!
  //analogWrite(DRIVE_RIGHT_PWM,120);
...
void PID_motor_control_l(float required_velocity, float current_velocity, float P, float D, float I)
{
  ...
  // Update the PWM
  analogWrite(DRIVE_LEFT_PWM,required_pwm);
  //analogWrite(DRIVE_LEFT_PWM,120);
